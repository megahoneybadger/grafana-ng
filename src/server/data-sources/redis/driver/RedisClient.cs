...
namespace CSRedis
{
	...
	public partial class RedisClient : IRedisClientSync, IRedisClientAsync
	{
		...
		/// <param name="socket"></param>
		/// <param name="endpoint"></param>
		/// <param name="asyncConcurrency"></param>
		/// <param name="asyncBufferSize"></param>
		internal RedisClient( IRedisSocket socket, EndPoint endpoint, int asyncConcurrency, int asyncBufferSize )
		{
			...
			_subscription.MessageReceived += (s, args) => SubscriptionReceived?.Invoke( this, args ); ;
			_subscription.Changed += ( s, args ) => SubscriptionChanged?.Invoke( this, args );
			_monitor.MonitorReceived += (s, args) => MonitorReceived?.Invoke( this, args );
			_connector.Connected += ( s, args ) => Connected?.Invoke( this, args );
			_transaction.TransactionQueued += ( s, args ) => TransactionQueued?.Invoke( this, args );
		}
